...
%
%
%   Known problems: IMPORTANTE! IMPORTANTE! IMPORTANTE! IMPORTANTE! IMPORTANTE!
...
doknockout=false; % Adicionei pois estava pondo 'true', devido ao nargin
...
if license('test','symbolic_toolbox')
    ...
    digits 4 % ERRRO! Preciso do SYMBOLIC MATH TOOLBOX aqui!
...
%         end
    ...
    if ischar(ind) && strcmpi(ind,'best') 
    ...
    elseif ischar(ind) && strcmpi(ind,'valbest')
        ...
        ref_tree=gpreformat(gp,treestrs)
         ...
         ref_tree=gpreformat(gp,gp.results.valbest.individual); % ref_tree = valbest bonito (infix)
        end
        
        % full_expr retorna a expressão da regressão, gene a gene.
        full_expr=sym(gp.results.valbest.returnvalues(1),'d')+sym(gp.results.valbest.returnvalues(2),'d')*sym(ref_tree{1});
        
        full_expr=expand(full_expr); % LINHA ADICIONAL!
        expr_array{1}=simplify(full_expr);
        % COMO ERA ANTES: expr_array{1}=simplify(full_expr)
        
        
        for i=2:length(ref_tree)
            gene_expr=sym(gp.results.valbest.returnvalues(i+1),'d')*sym(ref_tree{i});
            gene_expr=expand(gene_expr); % LINHA ADICIONAL!
            full_expr=full_expr+gene_expr;
            expr_array{i}=simplify(gene_expr);
        end
        
        gp.andre = char(full_expr);
    ...
    else % INDIVÍDUOS QUAISQUER!!! INDIVÍDUOS QUAISQUER!!! INDIVÍDUOS QUAISQUER!!!
...
%         end
        ...
        ref_tree=gpreformat(gp,treestrs)
         ...
         ref_tree=gpreformat(gp,gp.pop{ind}); % ref_tree = valbest bonito (infix)   
        end
        
       
        % full_expr retorna a expressão da regressão, gene a gene.
%         full_expr=sym(gp.results.valbest.returnvalues(1),'d')+sym(gp.results.valbest.returnvalues(2),'d')*sym(ref_tree{1})
        full_expr=sym(1,'d')+sym(1,'d')*sym(ref_tree{1}); % SÓ O BETA_0= E O 1o GENE!!!!! :-)
...
%         expr_array{1}=simplify(full_expr)
        ...
        for i=2:length(ref_tree)
            gene_expr=sym(1,'d')*sym(ref_tree{i});
%             gene_expr=expand(gene_expr) % LINHA ADICIONAL!
            full_expr=full_expr+gene_expr;
            expr_array{i}=simplify(gene_expr);
        end
            full_expr=expand(full_expr);
...
%         error('Illegal argument')
    ...
    disp(' ');
    full_expr_simp=char(full_expr); % IMPORTANTE(1)
    ...
    if nargout<1 % 
        ...
        if length(expr_array)>1
            ...
            pretty(expr_array{1}); % pretty: PRINT a symbolic expression.
            disp(' ');
...
%     end
    ...
    if nargout>0
        expr_sym=full_expr_simp; % AQUI! AQUI! AQUI!: cell_expr_sym
        gene_latex_expr=ones(1);
        full_latex_expr=ones(1);
        cell_expr_sym=ones(1);
    end

%     if nargout >3
%         cell_expr_sym=expr_array; % AQUI! AQUI! AQUI!: expr_sym
%     end
