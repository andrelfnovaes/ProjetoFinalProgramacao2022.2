...
% expr = mum ou dad
x=strrep(expr,'[-','[#'); % p/ o caso de ctes negativas
...
if node_type==0 || node_type==6 %pick any node
    ...
    % temp_ind = índices das letras no nó (ou gene ou indivíduo)

    %add indices of constant locations
    ind=[temp_ind findstr(x,'[')]; % Veja se há constantes
...
if ~num_nodes
    ...
    position=ind(fun_choice); % position e ind: função das letras!
end
