...
%get INDICES of nodes of any type: OK!
ind=picknode(expr,6,gp); % expr = INDIVÍDUO ; "6": indices of ALL NODES, sorted left to right
if isempty(ind)
...
try
    ...
    % args = #argumentos da operação escolhida em expr(ind)
    if isempty(args)
...
elseif args>2
    %get subtree rooted in this node
    [main_tree,sub_tree]=extract(ind,expr); % expr = INDIVÍDUO
    node=sub_tree(1);
    ...
    ind=picknode(expr,6,gp); % Se node_type = 6, picknode retorna os índices de TODOS os nós
    %get subtree rooted in this node
    [main_tree,sub_tree]=extract(ind,expr); % sub_tree <- expr
    node=sub_tree(1);

    %get index of 2nd node in 'sub_tree' (i.e. first logical argument)
    keynode_ind=picknode(sub_tree,6,gp); % Se node_type = 6, picknode retorna os índices de TODOS os nós
    keynode_ind=keynode_ind(2);

    %EXTRACT the 1st argument expression
    [remainder,arg1]=extract(keynode_ind,sub_tree);

    %EXTRACT the 2nd argument expression from the remainder
    % find the 1st node after $ in the remainder as this will be the
    % keynode of the 2nd argument we wish to extract
    keynode_ind=picknode(remainder,6,gp);
    token_ind=findstr(remainder,'$');

    hib=find(keynode_ind>token_ind); % find retorna vetor de índices
    keynode_ind=keynode_ind(hib);
    keynode_ind_1=keynode_ind(1);
    [remainder2,arg2]=extract(keynode_ind_1,remainder);
    ...
    %process arguments of these arguments if any exist: RECURSÃO! RECURSÃO! RECURSÃO!
    [rec1,rec1_status]=pref2inf(arg1,gp); % NÃE ENTREI AQUI (02.12.13 - 2a)
    [rec2,rec2_status]=pref2inf(arg2,gp); % NÃE ENTREI AQUI (02.12.13 - 2a)
