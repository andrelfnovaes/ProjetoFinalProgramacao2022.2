...
if gp.fitness.minimisation
    ...
    best_fitness=min(gp.fitness.values); % melhor fitness
...
% considered "best".
best_inds=find(gp.fitness.values==best_fitness); % índice do(s) best(s)
nodes=gp.fitness.numnodes(best_inds); % #nós do best
[lnodes,ind]=min(nodes); % lnodes = #nós do best ; ind = 1
best_ind=best_inds(ind); % só no caso de ter 2 ou mais bests



%store best of current population
gp.state.best.fitness=best_fitness; % fitness
gp.state.best.individual=gp.pop{best_ind}; % indivíduo = f(genes)
gp.state.best.returnvalues=gp.fitness.returnvalues{best_ind}; % Betas dos genes do best
...
if gp.fitness.terminate
    %%% IMPORTANTE!!! : terminate_value = tol de fitness %%%
    if gp.fitness.minimisation 
