...
for i=1:length(gp.nodes.functions.name)
    ...
    arity=nargin(gp.nodes.functions.name{i}); % #parâmetros de cada operação. Ex: arity(times)=2
...
if gp.nodes.functions.num_active>22 % IMPORTANTE: # máximo de operações é 22.
...
for i=1:gp.nodes.functions.num_active
    ...
    afid(i)=char(charnum+i+skip); % afid = abcdf, por exemplo (é uma string corrida)
end
...
[gp.nodes.functions.active_name_UC]=upper(temp); % upper -> letras Maiúsculas
...
active_ar=(gp.nodes.functions.arity(gp.nodes.functions.active)); % = só arity's ativados
