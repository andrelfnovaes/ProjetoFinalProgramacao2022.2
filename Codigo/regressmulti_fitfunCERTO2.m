...
if ~gp.state.run_completed %i.e. only calc. weighting coeffs during an actual run
    ...
    elseif gp.runcontrol.tipo==0 % REGRESSÃO! REGRESSÃO! REGRESSÃO! REGRESSÃO!
    ...
    elseif gp.runcontrol.tipo==1 % CLASSIFICAÇÃO! CLASSIFICAÇÃO! CLASSIFICAÇÃO! 
        
        gene_outputs2=gene_outputs(:,2:end);
        [theta,~,stats]=mnrfit(gene_outputs2,y);
...
elseif gp.runcontrol.tipo==1 % CLASSIFICAÇÃO!
    y_prob_train=exp(gene_outputs*theta)./(1+exp(gene_outputs*theta));
    if gp.runcontrol.num_classes==2
        ypredtrain=round(y_prob_train);
        ind=find(ypredtrain==0);
        ypredtrain(ind)=2;
    elseif gp.runcontrol.num_classes==3
    end
end
...
if gp.runcontrol.tipo==0 % REGRESSÃO!
    ...
    % MÉDIA de Significância DOS REGRESSORES ESTAT. SIGNIF. (t-stat)
...
elseif gp.runcontrol.tipo==1 % CLASSIFICAÇÃO!
    ...
    reg_pvalores=stats.p;
    reg_pvalores=reg_pvalores(2:end,:);
    reg_signif=find(reg_pvalores<alfa);
    
    % NÚMERO de regressores significantes (quantidade)
    gp.geracoes.numreg_signif(gp.geracoes.pop_size,gp.geracoes.num_geracoes)=length(reg_signif);
    
    % PROPORÇÃO de regressores significantes
    gp.geracoes.prop_numreg_signif(gp.geracoes.pop_size,gp.geracoes.num_geracoes)=length(reg_signif)/length(reg_pvalores);
    
    % MÉDIA de Significância
    gp.geracoes.media_signif(gp.geracoes.pop_size,gp.geracoes.num_geracoes)=mean(reg_pvalores(reg_signif));
    
    % Somente Regressores Significativos
    aux1=reg_pvalores<alfa;
    gp.geracoes.index_reg_signif{gp.geracoes.pop_size,gp.geracoes.num_geracoes}=aux1';
    
    % Regressores INsignificantes
    aux2=reg_pvalores>alfa;
    gp.geracoes.index_reg_INsignif{gp.geracoes.pop_size,gp.geracoes.num_geracoes}=aux2';
...
if gp.runcontrol.tipo==0 % REGRESSÃO!
    ...
    gp.geracoes.rmse(gp.geracoes.pop_size,gp.geracoes.num_geracoes)=rmse; % ANTIGO gp.geracoes.rmse
...
elseif gp.runcontrol.tipo==1 % CLASSIFICAÇÃO!
    ...
    fitness=1-p_acertos; % %erros
    gp.geracoes.rmse(gp.geracoes.pop_size,gp.geracoes.num_geracoes)=fitness; % %erros
    ...
    if p_acertos>=0.9999
        ...
        for i=1:num_genes
            ind=i+1;
            eval(['gene_outputs(:,ind)=' evalstr{i} ';']) % atualiza GENE_OUTPUTS com os cálculos de EVALSTR 
            %check for nonsensical answers and break out early with an 'inf' if so
            if  any(isnan(gene_outputs(:,ind))) || any(isinf(gene_outputs(:,ind)))
                fitness=Inf;
                return
